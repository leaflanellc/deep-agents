...
export const ThreadHistorySidebar = React.memo<ThreadHistorySidebarProps>(
  ({ open, setOpen, currentThreadId, onThreadSelect, currentAgentId }) => {
    const [threads, setThreads] = useState<Thread[]>([]);
    const [isLoadingThreadHistory, setIsLoadingThreadHistory] = useState(true);
    const { session } = useAuthContext();
    const deployment = useMemo(() => getDeployment(currentAgentId), [currentAgentId]);

    const fetchThreads = useCallback(async () => {
      if (!deployment?.deploymentUrl || !session?.accessToken) return;
      setIsLoadingThreadHistory(true);
      try {
        const client = createClient(session.accessToken, currentAgentId);
    ...
    }, [deployment?.deploymentUrl, session?.accessToken, currentAgentId]);
    ...
    const handleDeleteThread = useCallback(
      ...
      [session?.accessToken, currentAgentId, currentThreadId, onThreadSelect],
    );
...
}>(({ thread, isActive, onClick, onDelete }) => {
  return (
    <div className={`${styles.threadItem} ${isActive ? styles.active : ""}`}>
      <button
        onClick={onDelete}
      ...
      <button onClick={onClick} className={styles.threadButton}>
